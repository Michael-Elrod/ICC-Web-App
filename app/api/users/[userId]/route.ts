...
export const DELETE = withDb(async (connection, request, params) => {
  ...
  return await withTransaction(connection, async () => {
    ...
    await connection.execute("DELETE FROM note WHERE created_by = ?", [userId]);
    await connection.execute("DELETE FROM material WHERE created_by = ?", [
      userId,
    ]);
    await connection.execute("DELETE FROM task WHERE created_by = ?", [userId]);
    await connection.execute("DELETE FROM phase WHERE created_by = ?", [
      userId,
    ]);
    await connection.execute(
    ...
    await connection.execute("DELETE FROM invite_code WHERE updated_by = ?", [
      userId,
    ]);
