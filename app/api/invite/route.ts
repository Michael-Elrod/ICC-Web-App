...
export const GET = withRole(['Owner', 'Admin'], async (connection, session) => {
  const [codes] = await connection.query("SELECT code FROM invite_code LIMIT 1");
...
export const POST = withRole(['Owner', 'Admin'], async (connection, session, request) => {
