    ...
    const fetchCurrentPrefs = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setStatus("error");
          setMessage(
            "Invalid or expired link. Please check your email for a valid unsubscribe link or log " +
  ...
  const handlePreferenceUpdate = async (newPref: NotificationPref) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus("success");
        setMessage(
          `Your notification preferences have been updated to ${newPref}.`,
        );
      } else {
        setStatus("error");
        setMessage(
