...
describe("CloseJobModal", () => {
  ...
  it("shows initial step with confirmation question", () => {
    ...
    expect(
      screen.getByText("Are you sure you want to close this job?"),
    ).toBeInTheDocument();
  });
  ...
  it("Continue advances to confirm step", async () => {
    ...
    expect(
      screen.getByText(/Closing a job will set the job status to Closed/),
    ).toBeInTheDocument();
  });
  ...
  it("clicking backdrop overlay closes modal", async () => {
    ...
    const overlay = screen
      .getByText("Are you sure you want to close this job?")
      .closest(".fixed")!;
