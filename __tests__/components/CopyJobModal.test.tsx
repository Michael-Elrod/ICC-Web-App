...
describe('CopyJobModal', () => {
  ...
  it('shows confirm step with job name', () => {
    render(
      <CopyJobModal isOpen={true} onClose={vi.fn()} jobName="Kitchen Remodel" />
    )
  ...
  it('Cancel on confirm step closes modal', async () => {
    ...
    render(
      <CopyJobModal isOpen={true} onClose={onClose} jobName="Test Job" />
    )
  ...
  it('Continue advances to date selection step', async () => {
    const user = userEvent.setup()
    render(
      <CopyJobModal isOpen={true} onClose={vi.fn()} jobName="Test Job" />
    )
  ...
  it('clicking backdrop closes and resets', async () => {
    ...
    const overlay = screen.getByText(/Are you sure you want to copy this job/).closest('.fixed')!
