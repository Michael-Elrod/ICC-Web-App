...
describe('getS3Url', () => {
  ...
  it('builds correct URL from fileName', () => {
    ...
    expect(url).toBe('https://my-bucket.s3.us-east-1.amazonaws.com/floorplans/test.jpg')
  })
})
...
describe('getKeyFromUrl', () => {
  ...
  it('extracts key from full S3 URL', () => {
    const key = getKeyFromUrl('https://my-bucket.s3.us-east-1.amazonaws.com/floorplans/test.jpg')
...
describe('deleteFloorPlan', () => {
  ...
  it('sends DeleteObjectCommand with correct params', async () => {
    ...
    await deleteFloorPlan('https://my-bucket.s3.us-east-1.amazonaws.com/floorplans/test.jpg')
  ...
  it('propagates S3 errors', async () => {
    ...
    await expect(
      deleteFloorPlan('https://my-bucket.s3.us-east-1.amazonaws.com/floorplans/test.jpg')
...
describe('uploadFloorPlans', () => {
  ...
  it('throws when S3_ACCESS_KEY_ID is missing', async () => {
    ...
    await expect(uploadFloorPlans([file], '1')).rejects.toThrow('S3_ACCESS_KEY_ID is not set')
  })

  it('throws when S3_SECRET_ACCESS_KEY is missing', async () => {
    delete process.env.S3_SECRET_ACCESS_KEY
    const file = createMockFile('test.jpg', 1024, 'image/jpeg')
    await expect(uploadFloorPlans([file], '1')).rejects.toThrow('S3_SECRET_ACCESS_KEY is not set')
  })
  ...
  it('uploads files and returns URLs', async () => {
    ...
    urls.forEach((url: string) => {
      expect(url).toContain('my-bucket.s3.us-east-1.amazonaws.com/floorplans/job-42-')
    })
  })
  ...
  it('cleans up on partial failure', async () => {
    ...
    await expect(uploadFloorPlans(files, '1')).rejects.toThrow('Failed to upload floor plans')
