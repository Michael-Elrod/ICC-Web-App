...
describe('withDb', () => {
  ...
  it('returns the handler response', async () => {
    const expectedResponse = NextResponse.json({ data: 'test' }, { status: 200 })
...
describe('withAuth', () => {
  ...
  it('passes session and connection to handler', async () => {
    ...
    expect(handler).toHaveBeenCalledWith(mockConnection, session, expect.any(Request), expect.anything())
  })
})
...
describe('withTransaction', () => {
  ...
  it('commits on success', async () => {
    const result = await withTransaction(mockConnection as any, async () => 'result')
...
describe('checkEmailExists', () => {
  ...
  it('returns true when email exists', async () => {
    ...
    const result = await checkEmailExists(mockConnection as any, 'test@example.com')
    expect(result).toBe(true)
  })

  it('returns false when email not found', async () => {
    mockExecute.mockResolvedValueOnce([[]])
    const result = await checkEmailExists(mockConnection as any, 'unknown@example.com')
