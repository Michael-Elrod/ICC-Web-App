...
describe('sendPasswordResetEmail', () => {
  ...
  it('includes reset link with token in body', async () => {
    ...
    const sentCommand = mockSend.mock.calls[0][0].params
    expect(sentCommand.Message.Body.Html.Data).toContain('/reset-password?token=abc123')
  })
  ...
  it('propagates SES errors', async () => {
    ...
    await expect(sendPasswordResetEmail('user@test.com', 'token')).rejects.toThrow('SES failure')
  })
})
...
describe('sendInvitationEmail', () => {
  ...
  it('includes sender name in body', async () => {
    ...
    const sentCommand = mockSend.mock.calls[0][0].params
    expect(sentCommand.Message.Body.Html.Data).toContain('John has invited you')
  })
  ...
  it('propagates SES errors', async () => {
    mockSend.mockRejectedValueOnce(new Error('SES failure'))
    await expect(sendInvitationEmail('new@test.com', 'CODE', 'John')).rejects.toThrow('SES failure')
  })
})
