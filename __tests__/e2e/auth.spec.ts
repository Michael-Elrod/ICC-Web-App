  ...
  test.describe("Login Flow", () => {
    ...
    test("successful login redirects to jobs page", async ({ page }) => {
      ...
      await Promise.race([
        ...
        expect(page.getByText(/error|invalid|incorrect/i)).toBeVisible({
          timeout: 10000,
        }),
      ]);
    });
...
test.describe("Protected Routes", () => {
  test("redirects unauthenticated users from /jobs to login", async ({
    page,
  }) => {
  ...
  test("redirects unauthenticated users from /calendar to login", async ({
    page,
  }) => {
  ...
  test("redirects unauthenticated users from /contacts to login", async ({
    page,
  }) => {
